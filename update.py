...
def cu_update():
    ...
    # st.write(result)
    df = pd.DataFrame(result, columns=['C_ID','E_ID','First_Name','Last Name','Qualification','Address','Locality','City','Email','Phone_NO'])
    ...
    if selected_result:
        ...
        c_fn = selected_result[0][2]
        c_ln = selected_result[0][3]
        c_qual = selected_result[0][4]
        c_address = selected_result[0][5]
        c_locate = selected_result[0][6]
        c_city = selected_result[0][7]
        c_email = selected_result[0][8]
        c_phone = selected_result[0][9]
    ...
    with col1:
        ...
        new_e_id = st.number_input("Employee ID: ",max_value = 2999,value=e_id)
    ...
    with col2:
        ...
        if st.button("Update Customer Details"):
            cu_edit(new_c_id,new_e_id,new_c_fn,new_c_ln,new_c_qual, new_c_address, new_c_locate, new_c_city, new_c_email,new_c_phone,c_id,e_id,c_fn,c_ln, c_qual, c_address, c_locate, c_city, c_email,c_phone)
            st.success("Successfully updated:: {} to :: {} and other details".format(c_fn, new_c_fn))

    result2 = cu_view_all_data()
    df2 = pd.DataFrame(result2, columns = ['C_ID','E_ID','First_Name','Last Name','Qualification','Address','Locality','City','Email','Phone_NO','DOP','Order_C_ID','Item_ID','Quantity','O_Date','O_Amount','Price'])
...
def em_update():
    ...
    # st.write(result)
    df = pd.DataFrame(result, columns = ['E_ID','MGR_ID', 'First_Name','Last_Name','GENDER','DOB','SALARY','ADDRESS','Phone_NO'])
    ...
    if selected_result:
        ...
        emgr_id = selected_result[0][1]
        e_fn= selected_result[0][2]
        e_ln= selected_result[0][3]
        e_gender = selected_result[0][4]
        e_salary = selected_result[0][6]
        e_dob = selected_result[0][5]
    ...
    df2 = pd.DataFrame(result2, columns = ['E_ID','MGR_ID','First_Name','Last_Name','GENDER','SALARY','DOB','ADDRESS','Phone_NO'])
    with st.expander("Updated Data"):
        st.dataframe(df2)
