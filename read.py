...
def cu_read():
    ...
    # st.write(result)
    df = pd.DataFrame(result, columns = ['C_ID','E_ID','First_Name','Last Name','Qualification','Address','Locality','City','Email','Phone_NO','DOP','Order_C_ID','Item_ID','Price','Quantity','O_Date','O_Amount'])
...
def su_read():
    ...
    # st.write(result)
    df = pd.DataFrame(result, columns = ['Supp_ID','Name','Address','Ship_ID','Cost_of_shipping','Mode_of_Travel','Date_Of_Shipment','Store_ID'])
